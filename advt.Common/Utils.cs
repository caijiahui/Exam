...
namespace advt.Common
{
    ...
    public class Utils
    {
        ...
        public static T Clone_Entity<T>(T t) where T : new()
        {
            T _t = new T();
            foreach (PropertyInfo _p in t.GetType().GetProperties())
            {
                object t_Value = _p.GetValue(t, null);

                string _name = _p.Name;

                PropertyInfo _t_PropertyInfo = _t.GetType().GetProperty(_name);
                if (_t_PropertyInfo != null)
                {
                    _t_PropertyInfo.SetValue(_t, t_Value, null);
                }
            }
            return _t;
        }
        ...
        public static T Clone_Entity<T>(T t, object _o) where T : new()
        {
            T _t = Common.Utils.Clone_Entity(t);

            foreach (PropertyInfo _p in _o.GetType().GetProperties())
            {
                string val = _p.GetValue(_o, null).ToString();

                string _name = _p.Name;
                PropertyInfo _t_PropertyInfo = _t.GetType().GetProperty(_name);
                if (_t_PropertyInfo != null)
                {
                    _t_PropertyInfo.SetValue(_t, val, null);
                    break;
                }
            }
            return _t;
        ...
        {
            return InArray(str, stringarray, true);
        }
        ...
        /// <param name="stringarray">字符串数组</param>
        /// <returns>判断结果</returns>
        public static bool InArray<T>(T val, System.Collections.Generic.List<T> list) where T : new()
        {
            return list.Exists(new Predicate<T>(
                delegate(T _t)
                {
                    return _t.GetType().GetProperties()[0].GetValue(_t, null)
                        .Equals(val.GetType().GetProperties()[0].GetValue(val, null));
                }));
        }
        ...
        public static bool InArray(string str, string stringarray)
        {
            return InArray(str, SplitString(stringarray, ","), true);
        }
        ...
        {
            return InArray(str, SplitString(stringarray, strsplit), true);
        }
        ...
        /// <param name="filetype">将文件输出时设置的ContentType(Excel:  application/vnd.ms-excel)</param>
        public static void ResponseFile(string filepath, string filename, string filetype)
        {
            Stream iStream = null;

            if (string.IsNullOrWhiteSpace(filetype)) filetype = "application/file";
        ...
        public static bool IsImgFilename(string filename)
        {
            ...
            return IsImgFilename_Ext(extname);
        }

        /// <summary>
        /// 判断文件后缀名是否为浏览器可以直接显示的图片文件名
        /// </summary>
        /// <param name="extname">文件后缀名</param>
        /// <returns>是否可以直接显示</returns>
        public static bool IsImgFilename_Ext(string extname)
        {
        ...
        /// <returns></returns>
        public static string GetEmailName(string strEmail)
        {
            return GetEmailName(strEmail, strEmail);
        }
        ...
        /// <param name="defaultName"></param>
        /// <returns></returns>
        public static string GetEmailName(string strEmail, string defaultName)
        {
            //string name = Regex.Match(strEmail, "(^[^ @]+)@", RegexOptions.None).Groups[1].Value;
            if (strEmail == null) return "";
            string name = strEmail.Substring(strEmail.Trim().LastIndexOf("@")) ;
        ...
        /// <param name="str">要判断字符串</param>
        /// <returns>判断结果</returns>
        public static bool IsSafeUserInfString(string str)
        {
        ...
        /// <param name="url">超级链接地址</param>
        /// <param name="extendPage">周边页码显示个数上限</param>
        /// <param name="defaultrewrite">当前版块是否使用URL重写</param>
        /// <returns>页码html</returns>
        public static string GetStaticPageNumbers(int curPage, int countPage, string url, string expname, int extendPage, int defaultrewrite)
        {
            int startPage = 1;
            int endPage = 1;
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetTrueDefaultPath()
        {
            string defaultPath = HttpContext.Current.Request.Path;
            if (defaultPath.LastIndexOf("/") != defaultPath.IndexOf("/"))
                defaultPath = defaultPath.Substring(defaultPath.IndexOf("/"), defaultPath.LastIndexOf("/") + 1);
            else
                defaultPath = "/";
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetRootUrl(string defaultPath)
        {
        ...
        /// <returns></returns>
        public static object GetValueByProp(object o, string PropertyName)
        {

            System.Reflection.PropertyInfo propertyInfo = o.GetType().GetProperty(PropertyName);
        ...
        /// <param name="PropertyName"></param>
        /// <returns></returns>
        public static string GetValueStringByProp(object o, string PropertyName)
        {
            object oo = GetValueByProp(o, PropertyName);
        ...
        /// 对一个Entity赋值
        /// </summary>
        /// <param name="to_E">复制到Entity</param>
        /// <param name="from_E">来自</param>
        public static void CopyEntity(object to_E, object from_E)
        {
            Type from_E_Type = from_E.GetType();
            Type to_E_Type = to_E.GetType();

            PropertyInfo[] from_E_Type_Properties = from_E_Type.GetProperties();

            for (int i = 0; i < from_E_Type_Properties.Length; i++)
            {
                PropertyInfo from_E_Type_Propertie = from_E_Type_Properties[i];

                PropertyInfo to_E_Type_Propertie
                    = to_E_Type.GetProperty(from_E_Type_Propertie.Name);

                if (to_E_Type_Propertie != null)
                {
                    to_E_Type_Propertie.SetValue(to_E_Type_Propertie
                        , from_E_Type_Propertie.GetValue(from_E, null), null);
                }
            }
        ...
        /// <returns></returns>
        public static string AppendParameter(string url, IEnumerable<KeyValuePair<string, string>> parameters)
        {
            ...
            foreach (var parameter in parameters)
            {
                builder
                    ...
                    .Append("=")
                    .Append(Utils.UrlEncode(parameter.Value ?? string.Empty))
                    .Append("&");
            }
